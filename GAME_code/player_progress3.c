...
void linkid(struct node** root, struct data f);
...
void main()
{
   ...
   fclose(fp1);
   FILE *file = fopen("bannedplayers.txt", "r");
    if (file == NULL) {
    ...
    for (long i = fileSize - sizeof(int); i >= 0; i -= sizeof(int))
     {
   ...
   file=fopen("reports.txt","r");
    while (fscanf(file, "%d", &number[i]) == 1) 
    {
      i++;
    }
    fclose(file);
    file=fopen("inverted.txt","w");
    for(;i>0;i--)
    {
     fprintf(file,"%d\n",number[i]);
    }
